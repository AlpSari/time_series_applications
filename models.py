...
class Network(nn.Module):
    ...
    def __init__(self, n_nodes_list =[1, 32, 1], activation_fn=torch.relu):
        ...
        self.layers = nn.ModuleList([nn.Linear(node_in, node_out) for node_in, node_out in
                                    zip(n_nodes_list[:-2], n_nodes_list[1:-1])])
        self.batchnorm_layers = nn.ModuleList([nn.BatchNorm1d(node_in) for node_in in n_nodes_list[1:-1]])
...
class LSTMWithEncoder(nn.Module):
    ...
    def forward(self, inputs, hist):
        ...
        if inputs.ndim == 2:
            ...
            combined = torch.cat((hiddens), dim=2) # [n_batch, n_sequence, n_hidden + n_features]
        ...
        return y_predict[:,-1][:, None] # Use the last output from the sequence as the output